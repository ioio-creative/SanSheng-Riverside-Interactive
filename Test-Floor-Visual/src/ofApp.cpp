...
void ofApp::setup(){
    ...
    //setup screenWidth and screenHeight variables instead of using ofGetWidth()/ofGetHeight(), for testing in OF_FULLSCREEN
    screenHeight = ofGetHeight();
    screenWidth = screenHeight / (float)floorCanvas.getHeight() * floorCanvas.getWidth();
}

//--------------------------------------------------------------
void ofApp::initStaticRings()
{
...
//--------------------------------------------------------------
void ofApp::updateEmitMode()    //unsed at the moment ? 
{
...
void ofApp::keyPressed(int key){
    ...
    if(key == '[')          //add 1 floor user at cursor for development
    {
        ...
        floorUserManager.addFloorUser((ofGetMouseX()-screenWidth/2)/ss,(ofGetMouseY()-ofGetHeight()/2)/ss);
    }
