...
void ofApp::setup(){
	...
	//kinect
	//SanShengKinectManager = KinectManager(CANVAS_WIDTH, CANVAS_HEIGHT / 3, MAX_PLAYERS, &KinectMapper);
...
void ofApp::updateGuiInspectorValues() {
	for (int i = 0; i < MAX_PLAYERS; i++){
		bodyPosInspect[i] = ofToString(SanShengKinectManager.bodyPositions[i].x) + ", " + ofToString(SanShengKinectManager.bodyPositions[i].y) + ", " + ofToString(SanShengKinectManager.bodyPositions[i].z);
	}
}
